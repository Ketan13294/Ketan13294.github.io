---
layout: post
title:  "SlicerROS2: A Research and Development Module for Image-Guided Robotic Interventions"
date:   2023-05-01 22:21:59 +00:00
image: /images/SlicerROS2.png
categories: research
author: "Kapi Ketan Mehta"
authors: "Laura Connolly, Aravind S. Kumar*, <strong>Kapi Ketan Mehta*</strong>, Lidia Al-Zogbi, Peter Kazanzides, Parvin Mousavi, Gabor Fichtinger, Axel Krieger, Junichi Tokuda, Russell H. Taylor, Simon Leonard, Anton Deguet"
venue: "arXiv"
arxiv: https://arxiv.org/abs/2509.19076
youtube: https://youtu.be/k3VGs0YaE3g?si=uILm1Tj-Sa_IJWr3
---
Developed decoupled dynamics of a small quadruped and a manipulator with the non-negligible dynamics and performed stable and robust loco-manipulation tasks using non-linear model predictive control.